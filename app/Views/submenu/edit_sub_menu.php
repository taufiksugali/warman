            ...
            <div class="row">
                <div class="col-md-12">
                    ...
                    <div class="card card-custom gutter-b example example-compact">
                        ...
                        <!--begin::Form-->
                        <form method="post" class="form" action="<?php echo base_url('submenu/update'); ?>">
                            <div class="card-body">
                                <div class="form-group">
                                    <label>Menu Name
                                    <span class="text-danger">*</span></label>
                                    <input type="hidden" name="id" value="<?= $sub_menu->id ?>" />
                                    <input type="text" name="submenu_name" value="<?= $sub_menu->submenu_name ?>" class="form-control <?= ($validation->getError('submenu_name')) ? 'is-invalid' : ''; ?>" placeholder="Enter sub name" />
                                ...
                                <div class="form-group">
                                    ...
                                    <select class="select form-control custom-select showproduct <?= ($validation->getError('menu_id')) ? 'is-invalid' : ''; ?>" value="<?= old('menu_id'); ?>" id="menu_id" name="menu_id">
                                        ...
                                        <?php if (@$menu) :
                                            foreach ($menu as $row) :
                                        ?>
                                        <option value="<?= $row->id ?>" <?php if($sub_menu->menu_id == $row->id){echo "selected";} ?>><?= $row->menu_name ?></option>
                                ...
                                <div class="form-group">
                                    <label>Controller
                                    <span class="text-danger">*</span></label>
                                    <input type="text" name="controller" value="<?= $sub_menu->controller ?>" class="form-control <?= ($validation->getError('controller')) ? 'is-invalid' : ''; ?>" placeholder="Enter controller name" />
                                    <?php if($validation->getError('controller')){ echo '<div class="invalid-feedback">'.$validation->getError('controller').'</div>'; } ?>
                                </div>
                                <div class="form-group">
                                    <label>Sub Menu Status
                                    <span class="text-danger">*</span></label>
                                        <select class="select2 select form-control custom-select <?= ($validation->getError('is_active')) ? 'is-invalid' : ''; ?>" value="<?= old('is_active'); ?>" id="is_active" name="is_active" >
                                            <option></option>
                                            <option value="1" <?php if($sub_menu->is_active == 1){echo "selected";} ?>>Active</option>
                                            <option value="0" <?php if($sub_menu->is_active == 0){echo "selected";} ?>>Inactive</option>
                                        </select>
