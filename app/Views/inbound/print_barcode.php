                ...
                <div class="row justify-content-center py-8 px-8 py-md-27 px-md-0">
                  <div class="col-12">
                    ...
                    <?php $i = 1;
                      ...
                      <?php 
                        $qty_good = intval($row->qty_good_in);
                        $qty_ngood = intval($row->qty_notgood_in);
                        $total_qty = $qty_good + $qty_ngood;
                        for($x = 0; $x < $total_qty; $x++){
                      ?>
                            <?php if($i == 1) { echo "<tr>" ;} ?>
                            <td align="center">
                                <!-- <small style="font-size: 20px"><b><?= @$row->material_detail_id ?></b></small><br> -->
                                <img style="width: 300px" alt='Barcode' src="<?= base_url('/barcode.php?codetype=code128&size=40&text='.@$row->material_detail_id.'&print=true') ?>">
                            </td>
                            <?php if($i == 4) { echo "</tr>" ;} 
                          $i++;
                          if($i == 4){ $i = 1; } ?>
