                        ...
                        <div class="card-body">
                            <table class="table table-hover table-bordered">
                                ...
                                <tr>
                                    ...
                                    <td width="75%"><?= date('d M Y', strtotime($data_inbound->inbound_doc_date)) ?></td>
                                </tr>
                                <tr>
                                    <td width="25%"><strong>Received By</strong></td>
                                    <td width="75%"><?= $data_inbound->inbound_rcv_by ?></td>
                                </tr>
                                <tr>
                            ...
                            <table class="table table-separate table-head-custom table-checkable" id="kt_datatable1">
                                ...
                                <tbody>
                                    ...
                                    ?>
                                            ...
                                            <td><?= $value['qty_real'].' '.$value['uom'] ?></td>
                                            <td><?= $value['qty_good'].' '.$value['uom'] ?></td>
                                            <td><?= $value['qty_not_good'].' '.$value['uom'] ?></td>
                                            </tr>
                                    <?php
                                            $inbound_detail2 = $value['detail'];
                                            foreach ($inbound_detail2 as $row) :
                                        
                                    ?>
                                    <tr style="background-color:#e4e4e5">
                                        <td class="text-center"></td>
                                        <td>Barcode : <?= $row->mat_detail_id?> <br/>(Batch: <?= $row->batch_no?> - exp. <?= date("m-d-Y", strtotime($row->expired_date))?>)</td>
                                        <td></td>
                                        <td></td>
                                        <td>
                                            <?php if($row->qty_good_in == null){
                                                    echo '-';
                                                } else {
                                                    echo $row->qty_good_in.' '.$row->uom_name;
                                                } 
                                            ?>
                                        </td>
                                        <td>
                                            <?php if($row->qty_notgood_in == null){
                                                    echo '-';
                                                } else {
                                                    echo $row->qty_notgood_in.' '.$row->uom_name;
                                                } 
                                            ?>
                                        </td>
                                    </tr>
