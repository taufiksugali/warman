...
class RakModel extends Model
{
    ...
    public function generate_id(){
        $lastId = $this->db->table($this->table)
                ->select('MAX(RIGHT(rak_id, 7)) AS last_id')
                ->get();
		$lastMidId = $this->db->table($this->table)
                ->select('MAX(MID(rak_id, 4, 2)) AS last_mid_id')
                ->get()
                ->getRow()
                ->last_mid_id;
		$midId = date('y');
    ...
    // Serverside
    public function all_rak($limit, $start, $col, $dir)
    {
        if(session()->get('warehouse_id') == 'POSLOG'){
            $query = $this->db->table($this->table)
                    ->select('rak.*, area_blok.blok_name, wh_area.wh_area_name, warehouse.wh_name')
    ...
    public function search_rak($limit, $start, $search, $col, $dir)
    {
        if(session()->get('warehouse_id') == 'POSLOG'){
            $query = $this->db->table($this->table)
                    ->select('rak.*, area_blok.blok_name, wh_area.wh_area_name, warehouse.wh_name')
    ...
    public function search_rak_count($search)
    {
        if(session()->get('warehouse_id') == 'POSLOG'){
            $query = $this->db->table($this->table)
                    ->select('rak.*, area_blok.blok_name, wh_area.wh_area_name, warehouse.wh_name')
